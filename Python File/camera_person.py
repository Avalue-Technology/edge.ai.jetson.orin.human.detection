...
class QuickReceiver():
    def __init__(self):
        ...
        ### AI ###
        strModelName = './model/tiny_person/QDEEP.OD.TINY.PERSON.CFG'
        self.m_bStartDetector = 1
        self.m_detector = c_void_p(0)
        self.m_nTrajectoryX = (c_ulong * 128)()
        self.m_nTrajectoryY = (c_ulong * 128)()
        self.m_nWidth = 1920
        self.m_nHeight = 1080
        ...
        QDEEP.QDEEP_CREATE_OBJECT_DETECT( QDeepGPUType.NVIDIA, 0 , QDeepObjectDetectConfigModel.QDEEP_OBJECT_DETECT_CONFIG_MODEL_CUSTOMIZED_LITE, strModelName.encode('utf-8'), 
                                         byref(self.m_detector), QDeepObjectDetectFlag.TRAJECTORY_TRACKING, None)
        ...
        if video_type == 0:
            video_path = "./model/tiny_person/people-counting.mp4"
             # 確保檔案存在
        ...
        while True:
            ...
            for i in range(self.nObjectSize.value):
                    ...
                    cv2.rectangle(frame, (self.m_pObjectList[i].nX, self.m_pObjectList[i].nY), (self.m_pObjectList[i].nX+self.m_pObjectList[i].nWidth, self.m_pObjectList[i].nY+self.m_pObjectList[i].nHeight), object_color, 3, cv2.LINE_AA)
